...
def newrecipe_post(request):
    try:
        ...
        recipe = user.recipe_set.create(name=request.POST['recipename'], difficulty=request.POST['difficulty'], tags=request.POST['tags'], description=request.POST['description'],instructions=request.POST['instructions'], preparationtime=request.POST['preparationtime'],cookingtime=request.POST['cookingtime'],image=request.POST['image'])
        print(request.POST.getlist('ingredientname[]'))
        for i in range(len(request.POST.getlist('ingredientname[]'))): 
            recipe.ingredients_set.create(name=request.POST.getlist('ingredientname[]')[i], amount = request.POST.getlist('ingredientamount[]')[i]+request.POST.getlist('ingredientmeasurement[]')[i])
        user.save()
